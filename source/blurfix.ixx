...
// The blur and sharpen effects render the frame into an offscreen target, blur it and stretch it
// back, and D3DDrv creates that target at a flat 512x512 - one buffer pixel per five screen pixels
// at 2560x1440, hence the block edges in the flashbacks. Three immediates decide it:
//
...
//   SizeX = SizeY = 512, written into the render interface as the bound target's size.
//
// The 128x128 buffer beside them is the blur kernel source, sampled with normalised coordinates,
// so it already scales.
//
...
// Width/height operands of the colour and depth create calls.
static std::array<std::pair<int32_t*, int32_t*>, 2> apBufferSize{};
...
{
    // PUSH 0x200 / PUSH 0x200 / PUSH ESI / CALL [ECX+0x50] - height then width, then the device,
    // then IDirect3DDevice8::CreateTexture. The depth surface is the same shape one slot along at
    ...
    if (patternColour.empty() || patternDepth.empty() || patternCreate.empty() || patternBind.empty())
    {
